...
function loadRazorpay(): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    script.src = RAZORPAY_SCRIPT; script.async = true;
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    fetch("/api/addresses").then((r) => r.json()).then((data) => {
      ...
      setAddresses(addrs);
      const preferred = addrs.find((a: Address) => a.isDefault) || addrs[0];
  ...
  const handleCopyUPI = () => {
    navigator.clipboard.writeText(STORE_UPI_ID).then(() => {
      setUpiCopied(true); toast.success("UPI ID copied"); window.setTimeout(() => setUpiCopied(false), 2500);
  ...
  const createOrder = async () => {
    ...
    return data.order as { id: string; orderNumber: string };
  };

  const openRazorpay = async (order: { id: string; orderNumber: string }) => {
    ...
    const checkout = new window.Razorpay({
      ...
      handler: async (response) => {
        ...
        try {
          ...
          setOrderResult(order); setPendingOrderId(null); clearCart(); setStep(3); toast.success("Payment successful");
        } catch (error) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!order) order = await createOrder();
