...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    if (body.regularPrice !== undefined && Number(body.regularPrice) <= 0) return NextResponse.json({ error: "Regular price must be greater than zero" }, { status: 400 });
    if (body.salePrice !== undefined && body.salePrice !== null && body.regularPrice !== undefined && Number(body.salePrice) >= Number(body.regularPrice)) return NextResponse.json({ error: "Sale price must be less than regular price" }, { status: 400 });
