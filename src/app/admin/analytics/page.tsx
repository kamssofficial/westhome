  ...
  return (<div className='space-y-6'>
    ...
    {loading ? (<div className='grid grid-cols-2 md:grid-cols-4 gap-3'>{Array.from({length:8}).map((_,i) => (<div key={i} className='bg-white rounded-xl border border-black/[.06] p-4 animate-pulse'><div className='h-8 w-8 bg-gray-100 rounded-lg mb-3'/><div className='h-6 w-16 bg-gray-100 rounded mb-1'/><div className='h-3 w-20 bg-gray-100 rounded'/></div>))}</div>) : data ? (<>
      ...
      {tab === 'realtime' && (<div className='space-y-4'>
        ...
        <div className='bg-white rounded-xl border border-black/[.06] p-6'><h2 className='font-semibold text-sm flex items-center gap-2 text-[#1a1917] mb-4'><Radio size={16} className='text-[#d4a574]' /> Visitors Online</h2>
          {(live.visitors||[]).length > 0 ? <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>{live.visitors.map((v) => (<div key={v.sessionId} className='border border-black/[.06] rounded-lg p-3'>
            <div className='flex items-center justify-between mb-1.5'><span className='text-xs font-semibold text-[#1a1917]'>{v.isCustomer ? 'Customer' : 'Visitor'}</span><span className='text-[10px] text-[#b0aba6]'>Active {v.secondsSinceActive < 60 ? v.secondsSinceActive + 's' : Math.floor(v.secondsSinceActive/60) + 'm'} ago</span></div>
            <p className='text-xs text-[#6b6560] truncate'>{v.viewingProduct ? 'Viewing: ' + v.viewingProduct : v.currentPage ? 'On: ' + v.currentPage : 'Browsing'}</p>
            <p className='text-[10px] text-[#b0aba6] capitalize mt-1'>{v.device}</p>
          </div>))}</div> : <p className='text-sm text-[#b0aba6] text-center py-6'>{live.live === 0 ? '0 visitors online' : 'Loading visitors…'}</p>}
        </div>
