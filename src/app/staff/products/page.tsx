          ...
          {products.map((product) => {
            ...
            return (
              <div key={product.id} className="bg-surface rounded-[1.35rem] border border-border overflow-hidden">
                ...
                <div className="p-3">
                  <p className="text-[10px] text-amber-600 font-medium uppercase tracking-wider">{product.category?.name || "—"}</p>
                  <h3 className="text-sm font-medium text-stone-900 mt-0.5 line-clamp-1">{product.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm font-semibold text-stone-900">₹{Number(product.regularPrice).toLocaleString()}</span>
                    {product.salePrice && (
                      <span className="text-xs text-stone-400 line-through">₹{Number(product.salePrice).toLocaleString()}</span>
                    )}
                  </div>
                </div>
              </div>
