...
export default function StaffProductEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState<any>({
    name: "", sku: "", description: "", shortDescription: "", regularPrice: "", salePrice: "",
    stockQuantity: "0", lowStockThreshold: "5", trackInventory: true, allowBackorder: false,
    categoryId: "", subcategoryId: "", status: "DRAFT", purchaseMethod: "BOTH",
    ...
    careInstructions: "", warranty: "", packagingType: "", packagingDimensions: "", includedItems: "",
    dimensionUnit: "cm", height: "", width: "", length: "", depth: "", diameter: "", weight: "", weightUnit: "kg", capacity: "", capacityUnit: "L",
    allowCustomSize: false, customSizeUnit: "cm", customSizeMinWidth: "", customSizeMinLength: "", customSizeMinHeight: "",
    customSizeMaxWidth: "", customSizeMaxLength: "", customSizeMaxHeight: "", customSizePricingMethod: "area", customSizeRequiresApproval: false,
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (!p) throw new Error(productData.error || "Product not found");
      const numberFields = ["regularPrice","salePrice","stockQuantity","lowStockThreshold","height","width","length","depth","diameter","weight","capacity","customSizeMinWidth","customSizeMinLength","customSizeMinHeight","customSizeMaxWidth","customSizeMaxLength","customSizeMaxHeight"];
      const next = { ...form, ...p };
      for (const key of numberFields) next[key] = p[key] === null || p[key] === undefined ? "" : String(p[key]);
      setForm(next);
      setCategories(categoryData.categories || []);
    }).catch(e => toast.error(e.message || "Failed to load product")).finally(() => setLoading(false));
  }, [id]);
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: any = { ...form };
      numeric.forEach(k => body[k] = k === "stockQuantity" || k === "lowStockThreshold" ? (body[k] === "" ? 0 : parseInt(body[k], 10)) : num(body[k]));
