...
import { memo, CATALOG_TTL_MS, NS } from "@/lib/memoCache";

// Build the filter facet set. `applyScope` narrows the base where clause for
// a category/subcategory page (empty string = shop-wide facets).
async function buildFilters(applyScope: (where: Record<string, unknown>) => void) {
  const where: any = { isActive: true, status: "ACTIVE" };
  applyScope(where);

  // Parallel efficient queries instead of loading all products
...
export async function GET(request: NextRequest) {
  ...
  // One memo key per scope; a previous version ran this 14-query fanout on
  // EVERY request with no cache — measured 1.2–2.8s responses repeating until
  // the instance warmed. 5-minute TTL + edge SWR bounds staleness for facets.
  const cacheKey = `${NS.products}:filters:${categorySlug}:${subcategorySlug}`;
  try {
    const payload = await memo(cacheKey, CATALOG_TTL_MS, () =>
      buildFilters((where) => {
        if (categorySlug) (where as any).category = { slug: categorySlug };
        if (subcategorySlug) (where as any).subcategory = { slug: subcategorySlug };
      }),
    );
