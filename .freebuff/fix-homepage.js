        ...
        {hero ? (
          <div className="relative rounded-2xl overflow-hidden h-[420px] md:h-[480px] bg-primary">
            ...
            <div className="relative h-full flex flex-col justify-end p-6 pb-8">
              {hero.subtitle && (
                <p className="text-[11px] font-semibold tracking-[0.15em] text-white/80 uppercase mb-1">{hero.subtitle}</p>
              )}
              <h1 className="font-display text-4xl md:text-5xl text-white leading-[1.05] mb-3">{hero.title || "West Home"}</h1>
              {hero.description && (
                <p className="text-white/70 text-sm mb-5 leading-relaxed whitespace-pre-line">{hero.description}</p>
              )}
      ...
      <section className="px-4 py-6">
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-2.5">
            {[1, 2, 3, 4].map((i) => (<div key={i} className="s
